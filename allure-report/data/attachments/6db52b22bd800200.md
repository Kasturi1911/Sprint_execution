...
```ts
  ...
  19 | 
> 20 |      await this.page.getByLabel('First Name').fill(user.firstName);
     |                                               ^ Error: locator.fill: Test timeout of 120000ms exceeded.
  21 |   await this.page.getByLabel('Last Name').fill(user.lastName);
  22 |   await this.page.getByLabel('Address').fill(user.address);
  23 |   await this.page.getByLabel('City').fill(user.city);
  24 |   await this.page.getByLabel('State').fill(user.state);
  25 |   await this.page.getByLabel('Zip Code').fill(user.zipCode);
  26 |   await this.page.getByLabel('Phone').fill(user.phone);
  27 |   await this.page.getByLabel('SSN').fill(user.ssn);
  28 |   await this.page.getByLabel('Username').waitFor({ state: 'visible' });
  29 |   await this.page.getByLabel('Username').fill(user.username);
  30 |   await this.page.getByLabel('Password').fill(user.password);
  31 |   await this.page.getByLabel('Confirm').fill(user.password);
  32 |   }
  33 | 
